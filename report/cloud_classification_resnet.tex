...
\begin{itemize}
    ...
    \item \textbf{Pooled Test Set ($N=3,330$ images)}: The sample-concatenated evaluation set. Because GCD accounts for 85.95\% of this pool ($2,862 / 3,330$), pooled combined accuracy is inherently GCD-heavy. Consequently, \textbf{source-balanced averaging} is used as the primary reported metric, treating both source domains with equal weight.
\end{itemize}
...
Table~\ref{tab:taxonomy_mapping} details the mapping between source labels and the five shared classes.
...
\end{itemize}
Residual shortcut connections propagate gradients directly across layers during backpropagation, stabilizing optimization across varying depths while providing a practical trade-off between model size and model capacity. The stock ImageNet classifier is replaced by a small head: dropout ($p=0.3$), a linear layer to 256 units with batch normalisation and a GELU activation, dropout ($p=0.2$), then a linear layer to the five classes. Dropout is applied only in this head, not in the residual backbone.
...
As supporting exploration, we evaluated ten hyperparameter configurations on the internal development validation set (excluding test samples) covering AdamW and SGD with momentum, learning rates, weight decay, label smoothing, and dropout. Figure~\ref{fig:family_convergence} displays convergence trajectories for the selected baseline configuration, while complete validation results across all ten screening runs are documented in Appendix Table~\ref{tab:tuning_results}.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.76\linewidth]{figures/resnet_family_gradient_descent_comparison.png}
\caption{Gradient descent convergence dynamics across ResNet-18, ResNet-34, and ResNet-50 during 5-epoch screening runs of the selected configuration on the internal development validation set. Panels display training loss convergence (top-left), unsmoothed validation loss (top-right), validation accuracy (bottom-left), and validation macro-F1 score (bottom-right). Legends indicate checkpoint-selected screening performance.}
\label{fig:family_convergence}
\end{figure}

The screening runs show that differential learning rates with AdamW ($\eta_{\text{bb}} = 5\times 10^{-5}$, $\eta_{\text{head}} = 5\times 10^{-4}$) outperform standard momentum SGD by 1.2 to 2.0 percentage points across architectures (+1.5~pp for ResNet-18, +2.0~pp for ResNet-34, and +1.2~pp for ResNet-50). The unregularized label smoothing setting ($\alpha=0.0$) with weight decay $\lambda=1\times 10^{-2}$ yielded stable convergence across all three architectures and was selected as the baseline configuration for production benchmarks.

Separate per-pool hyperparameter tuning was run for the CCSN-only and GCD-only pools; the joint pool kept its selected configuration. The CCSN pool favoured a more aggressive AdamW setting (higher learning rates, label smoothing 0.1) for ResNet-18 and a higher weight decay ($5\times 10^{-2}$) for ResNet-34 and ResNet-50. The GCD pool matched the joint configuration for all three architectures. Appendix~\ref{sec:appendix_tuning} lists the per-pool settings and validation scores.
...
\subsection{Source-Balanced Joint Performance}

The primary result is the joint CCSN+GCD model on the held-out test set, reported with source-balanced averaging across the two sensor types. Table~\ref{tab:primary_results} presents this primary comparison.
...
Cross-source transfer is lossy in both directions: top-1 accuracy is about 35--41\% under transfer, compared with about 90\% for in-domain GCD evaluation. Both directions lose most of the in-domain accuracy; per-direction and per-class values are in Tables~\ref{tab:transfer_summary} and~\ref{tab:transfer_per_class}. The CCSN-only models here use the CCSN-tuned configuration, which transfers less well than a CCSN model trained with the joint-pool settings.
...
\subsection{CCSN In-Domain Performance Across Training Configurations}
...
\bottomrule
\end{tabular}
\caption{CCSN in-domain top-1 accuracy under three training configurations, using the minimum-validation-loss checkpoint and three-seed mean $\pm$ across-seed SD.}
\label{tab:ccsn_configs}
\end{table}
...
Table~\ref{tab:tuning_results} documents empirical validation performance across the ten joint-pool hyperparameter exploration configurations evaluated on the internal development validation set ($N=2,663$ images). Sweeps explored differential learning rates, weight decay, label smoothing, and dropout regularization under AdamW and SGD with momentum. The last configuration listed (AdamW, standard differential learning rates, no label smoothing) was selected for the joint production runs.
...
\toprule
\textbf{Architecture} & \textbf{Pool} & \textbf{Configuration} & \textbf{Best Val Loss} & \textbf{Best Val Acc.} & \textbf{Best Val Macro-F1} \\
\midrule
ResNet-18 & CCSN & Aggressive AdamW & 1.0460 & 60.43 & 56.98 \\
ResNet-18 & GCD & AdamW, no label smoothing & 0.2609 & 88.60 & 90.77 \\
ResNet-34 & CCSN & AdamW, high weight decay & 1.0231 & 60.96 & 58.39 \\
ResNet-34 & GCD & AdamW, no label smoothing & 0.2678 & 89.25 & 91.03 \\
ResNet-50 & CCSN & AdamW, high weight decay & 1.0029 & 60.96 & 57.16 \\
ResNet-50 & GCD & AdamW, no label smoothing & 0.2459 & 89.86 & 91.66 \\
\bottomrule
\end{tabular}% 
}
\caption{Per-pool winning trials used by the single-source production configs. Validation metrics are from the per-pool tuning sweeps.}
